...
export const getServerSideProps = async (ctx) => {
  ...
  const res = await fetch(
    `https://teststrapi1.ktern.com/news-center-pages?slug=${ctx.params.news}`,
    {
      method: "get",
    }
  );
  ...
  const news = await fetch(
    `https://teststrapi1.ktern.com/news-center-pages?_sort=updatedAt:desc&slug_nin=${ctx.params.news}&_limit=4`
  );
